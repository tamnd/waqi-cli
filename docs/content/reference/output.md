...
describes what users get. Pick a format with `-o`, or let waqi choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
waqi <command> -o table   # aligned columns for reading
waqi <command> -o jsonl   # one JSON object per line, for piping
waqi <command> -o json    # a single JSON array
waqi <command> -o csv     # spreadsheet friendly
waqi <command> -o tsv     # tab-separated
waqi <command> -o url     # just the URL column
waqi <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
waqi <command> --fields id,title,url
```
...
```bash
waqi <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
waqi <command>            # a table, because this is a terminal
waqi <command> | wc -l    # JSONL, because this is a pipe
```
