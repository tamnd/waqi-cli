// Package cli assembles the waqi command tree from the waqi
...
// NewApp assembles the kit application from the waqi domain. The
...
//
// To add a command, declare it in waqi/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := waqi.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(waqi.Domain{}).Register(app)
